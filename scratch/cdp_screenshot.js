...
async function capture() {
    ...
    console.log('1. Capturing Live Dashboard Screenshot...');
    const shot1 = await cdpRequest(pageTarget.id, 'Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('docs/demo_live_dashboard.png', Buffer.from(shot1.data, 'base64'));

    // 2. Flight Cards View (Scroll down & toggle Cards mode)
    ...
    fs.writeFileSync('docs/demo_live_cards.png', Buffer.from(shotCards.data, 'base64'));

    // 3. Timeline View
    console.log('3. Capturing Live Timeline Screenshot...');
    await cdpRequest(pageTarget.id, 'Runtime.evaluate', {
        expression: `
            window.scrollTo(0, 0);
    ...
    fs.writeFileSync('docs/demo_live_timeline.png', Buffer.from(shot2.data, 'base64'));

    // 4. Coordination Engine Tab
    console.log('4. Capturing Coordination Engine Screenshot...');
    await cdpRequest(pageTarget.id, 'Runtime.evaluate', {
        expression: `
            const btnCoord = document.querySelector('[data-maintab="coordination"]');
            if (btnCoord) btnCoord.click();
        `
    });
    await new Promise(r => setTimeout(r, 2000));
    const shotCoord = await cdpRequest(pageTarget.id, 'Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('docs/demo_live_coordination.png', Buffer.from(shotCoord.data, 'base64'));

    // 5. Add Flight Screen
    console.log('5. Capturing Add Flight Screen Screenshot...');
    await cdpRequest(pageTarget.id, 'Runtime.evaluate', { expression: `window.location.hash = '#add-flight'` });
    await new Promise(r => setTimeout(r, 1500));
    const shot3 = await cdpRequest(pageTarget.id, 'Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('docs/demo_live_add_flight.png', Buffer.from(shot3.data, 'base64'));

    chromeProcess.kill();
    console.log('🎉 All 5 Live Real Screenshots Captured Successfully!');
}
